...
# Apadts learning rate according to epoch
...
def Conv_bn_relu(infor, **conv_params):
    ...
    # Create Conv layer and apply to input tensor
    conv = Conv2D(filters=filters, kernel_size=kernel_size,
                  strides=strides, padding=padding,
                  kernel_initializer=kernel_initializer,
                  kernel_regularizer=kernel_regularizer)(infor)
    
    # Create BN layer and apply to conv layer
    norm = LayerNormalization(axis=CHANNEL_AXIS)(conv)

    # Create relu layer and apply to norm layer
...
model.fit_generator(datagen.flow(X_train, Y_train, batch_size=Config.batch_size),
                        ...
                        epochs=Config.nb_epoch*2, verbose=1, 
...
model.fit_generator(datagen.flow(X_train, Y_train, batch_size=Config.batch_size),
                        steps_per_epoch=X_train.shape[0] // Config.batch_size,
                        validation_data=(X_test, Y_test),
                        epochs=Config.nb_epoch*10, verbose=1, 
                        callbacks=[lr_reducer, early_stopper, csv_logger])   
